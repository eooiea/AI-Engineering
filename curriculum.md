...
* **실습**: [examples/context_engineering_example.py](file:///c:/Coding/AI-Engineering/examples/context_engineering_example.py) 구동을 통한 80%+ 토큰 절감 및 캐싱 시뮬레이션.
...
* **실습**: [examples/mcp_server.py](file:///c:/Coding/AI-Engineering/examples/mcp_server.py) FastMCP 기반 도구/리소스/프롬프트 서버 구축 및 IDE 연동.
...
* **실습**: [examples/structured_outputs_example.py](file:///c:/Coding/AI-Engineering/examples/structured_outputs_example.py) 에러 피드백 주입을 통한 자가 코드 수정 파이프라인.
...
* **실습**: [examples/agent_memory_example.py](file:///c:/Coding/AI-Engineering/examples/agent_memory_example.py) 세션 간 엔티티 기억 및 대화 압축 시뮬레이터.
...
* **실습**: [examples/hitl_example.py](file:///c:/Coding/AI-Engineering/examples/hitl_example.py) 위험 도구 감지 시 일시 정지(Interrupt) 및 관리자 승인(Approval) 재개.
...
* **이론**: Router, Supervisor-Worker, Handoff Swarm 패턴, LangGraph 스타일의 StateGraph 및 조건부 엣지(Conditional Edges).
* **실습**: [examples/orchestrator/orchestrator.py](file:///c:/Coding/AI-Engineering/examples/orchestrator/orchestrator.py) 기획자-작성자-검증자 상태 전이 파이프라인.
...
* **실습**: [examples/rag_example.py](file:///c:/Coding/AI-Engineering/examples/rag_example.py) RRF 검색 융합 및 Re-ranking을 거친 사실 기반 답변 합성.
...
* **실습**: [examples/harness/eval_harness.py](file:///c:/Coding/AI-Engineering/examples/harness/eval_harness.py) 회귀 방지 자동화 채점 하네스.
...
* **실습**: [examples/guardrails_example.py](file:///c:/Coding/AI-Engineering/examples/guardrails_example.py) 주입 공격 차단 및 개인정보 마스킹 파이프라인.
...
* **실습**: [examples/observability_example.py](file:///c:/Coding/AI-Engineering/examples/observability_example.py) 파이프라인 단계별 Trace/Span 자동 로깅 및 집계.
...
├── examples/                    <-- 실무 파이썬 실습 코드 폴더
│   ├── context_engineering_example.py  <-- Module 0: Prompt Caching & 토큰 다이어트
│   ├── mcp_server.py                   <-- Module 1: FastMCP 표준 서버 (Tools/Resources/Prompts)
│   ├── structured_outputs_example.py   <-- Module 3: Pydantic 스키마 & Self-Healing
│   ├── agent_memory_example.py         <-- Module 4: Short/Long-term Entity Memory
│   ├── hitl_example.py                 <-- Module 5: Human-in-the-Loop & Breakpoint
│   ├── orchestrator/
│   │   └── orchestrator.py             <-- Module 6: Multi-Agent StateGraph
│   ├── rag_example.py                  <-- Module 7: Hybrid RAG & Re-ranking
│   ├── harness/
│   │   └── eval_harness.py             <-- Module 8: Evaluation Harness & LLM Judge
│   ├── guardrails_example.py           <-- Module 9: Enterprise Guardrails
│   └── observability_example.py        <-- Module 10: OpenTelemetry Tracing
